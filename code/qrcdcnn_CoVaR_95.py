...
#     next = Conv1D(1, 7, activation="linear")(x)
#     cnn = Model(input_ts, next)
#     cnn.compile(optimizer="adadelta", loss=lambda y_true,y_pred: quantile_loss(varlevel,y_true,y_pred))
#     history =cnn.fit(trainX, trainY,epochs=NUM_EPOCHS,batch_size=BATCH_SIZE,shuffle=True,validation_data=(testX,testY))
#     return cnn,history
...
def model(trainX, trainY, testX, testY):
    ...
    input_ts = Input(shape=(trainX.shape[1], 1))
    x = Conv1D(filters, kernel_size, activation="relu", padding="causal", dilation_rate=1, kernel_regularizer=reg)(input_ts)
    x = Conv1D(filters, kernel_size, activation="relu", padding="causal", dilation_rate=2, kernel_regularizer=reg)(x)
    x = Conv1D(filters, kernel_size, activation="relu", padding="causal", dilation_rate=4, kernel_regularizer=reg)(x)
    x = Conv1D(filters, kernel_size, activation="relu", padding="causal", dilation_rate=8, kernel_regularizer=reg)(x)
    x = Conv1D(filters, kernel_size, activation="relu", padding="causal", dilation_rate=16, kernel_regularizer=reg)(x)
    x = Conv1D(filters, kernel_size, activation="relu", padding="causal", dilation_rate=32, kernel_regularizer=reg)(x)
    x = MaxPooling1D(pool_size=2, strides=1)(x)
    next = Conv1D(1, 7, activation="linear")(x)
    cnn = Model(input_ts, next)
    cnn.compile(optimizer="adadelta", loss=lambda y_true, y_pred: quantile_loss(varlevel, y_true, y_pred))
